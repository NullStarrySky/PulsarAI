...
- Conversation generation keeps a fixed startup sequence in `Resources/Conversation/application/conversation-generation.ts`: create the blank assistant target, build the minimal authorized runtime environment, index Plugin containers and explicit references, compile the root `context.imd`, then hand control to root `generation.js` or the default Agent.
- The immutable `PulsarAI` character package owns project-Agent conversations. Persist the selected normal character package on `Conversation.projectPackageId`; keep project CRUD scoped through `Agent/application/project-agent-runtime.ts`, and let the package-local built-in project plugin supply the role-playing architecture context to the existing ToolLoopAgent.
...
- Plugin contents are a nested `root` file tree. Derive file semantics from suffixes, use root `info.md` as documentation, root `generation.js` as the optional process, root `context.imd` as the role-aware entry document, and ordinary `action/` and `background/` folders as runtime conventions rather than fixed containers.
- Plugin resources never become ambient Sandbox variables. Cross-resource access must use `<@container-name>` for an unambiguous visible container or explicit `<@local:...>`, `<@path:...>`, `<@id:...>`, and `<@container:root|plugin|global/name>` references. Containers are declared inside resource source, may import existing containers under aliases, remain lazy namespaces, and must report collisions, missing references, and cycles instead of applying last-write-wins merges.
...
- Conversation actions are JavaScript files under the conventional Plugin `action/` folder. The composer stores at most one leading `ActionPart` on the user message; generation adds `action` and `prompt` to the minimal Sandbox environment, and an invoked action temporarily replaces root `generation.js` for that run.
