...
- Plugin background resources use media content (`kind: "media"`) with an image or video URL. Keep media parsing, preview, and built-in background assets under `Resources/Plugin` rather than encoding backgrounds as CSS snippets.
- Insertable Plugin resources persist `insertPosition`, `insertDepth`, and structured `insertCondition` rows. Keep built-in condition functions and their runtime environment in `Resources/Plugin/application/plugin-condition-environment.ts`; custom rows may execute through the existing Sandbox, but UI should not revert to raw multiline condition strings.
- Conversation actions are JavaScript resources in the fixed Plugin `action` container. The composer stores at most one leading `ActionPart` on the user message; generation adds `action` and `prompt` to the normal Sandbox environment, and an invoked action may temporarily override only that run when its resource is marked inserted and passes the normal conditions.
...
- plugin resources as normal workspace resources with right-sidebar control and a compact composer menu;
