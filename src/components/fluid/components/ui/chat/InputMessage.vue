...
export interface InputMessageProps {
	...
	suggestions?:
		| Array<string | InputMessageSuggestionItem>
...
const emit = defineEmits<{
	...
	(
		e: "selectSuggestion",
		suggestion: string | InputMessageSuggestionItem,
	): void;
}>();
...
const suggestionGroups = computed<InputMessageSuggestionGroup[]>(() => {
	...
	return [
		{ suggestions: source as Array<string | InputMessageSuggestionItem> },
	];
});
const suggestionsArr = computed(() =>
	suggestionGroups.value.flatMap((group) => group.suggestions),
);
...
const hasSuggestionLabels = computed(() =>
	suggestionGroups.value.some((group) => group.label),
);
const suggestionsOpen = computed(() => suggestionsArr.value.length > 0);
...
function useRegionHeight() {
	...
	const setRef = (el: any) => {
		const rawEl =
			(el?.$el as HTMLElement | null) ?? (el as HTMLElement | null) ?? null;
		ro?.disconnect();
		ro = null;
		if (!rawEl) return;
		const sync = () => {
...
function handleKeyDown(e: KeyboardEvent) {
	...
	if (
		...
		if (el) {
			...
			if (
				e.key === "ArrowUp" &&
				!currentValue.value.slice(0, caret).includes("\n")
			) {
				const start =
					historyIndex.value == null ? hist.length : historyIndex.value;
				if (start > 0) {
					e.preventDefault();
					if (historyIndex.value == null)
						draftBeforeHistory.value = currentValue.value;
					const ni = start - 1;
    ...
    <AnimatePresence v-if="suggestionsArr.length > 0" :initial="false">
      ...
      >
        <div
          :ref="
            (el) => {
              suggestionListRef = el as HTMLDivElement | null;
