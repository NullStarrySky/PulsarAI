...
<template>
  <div class="grid gap-3 px-4 py-4 sm:grid-cols-[minmax(13rem,1fr)_minmax(14rem,28rem)]">
    <div class="min-w-0 space-y-1.5">
      <div class="block text-sm font-medium leading-5">
        <span v-if="required" class="text-destructive">*</span>
        {{ title }}
      </div>
      <div v-if="description" class="block max-w-[42rem] text-[13px] leading-5 text-muted-foreground">{{ description }}</div>
    </div>
