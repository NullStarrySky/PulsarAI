...
import { AZURE_TTS_API_KEY_SECRET, type AzureTtsSettings } from "../domain/tts";
...
export async function listAzureSpeechVoices(settings: AzureTtsSettings, signal?: AbortSignal) {
  const response = await azureSpeechFetch(settings, "/cognitiveservices/voices/list", {
    method: "GET",
    signal,
  });
  ...
  return payload.flatMap((voice): AzureSpeechVoice[] => voice.ShortName ? [{
  ...
  }] : []).sort((left, right) => left.locale.localeCompare(right.locale) || left.localName.localeCompare(right.localName));
}
...
export async function synthesizeWithAzureSpeech(options: {
  ...
  const body = buildSsml(text, locale, voiceId, options.style?.trim(), options.speed);
  const response = await azureSpeechFetch(options.settings, "/cognitiveservices/v1", {
    method: "POST",
    headers: {
...
function azureSpeechFetch(settings: AzureTtsSettings, path: string, init: RequestInit) {
  const region = settings.region.trim().toLowerCase();
  if (!/^[a-z0-9-]+$/.test(region)) throw new Error("请填写有效的 Azure Speech Region。");
  const headers = new Headers(init.headers);
  headers.set("Ocp-Apim-Subscription-Key", `<<${AZURE_TTS_API_KEY_SECRET}>>`);
  return modelProxyFetch(`https://${region}.tts.speech.microsoft.com${path}`, { ...init, headers });
}

function buildSsml(text: string, locale: string, voiceId: string, style?: string, speed?: number) {
  const percentage = speed && speed !== 1 ? Math.round((speed - 1) * 100) : 0;
  const rate = percentage ? `${percentage > 0 ? "+" : ""}${percentage}%` : "default";
...
function mediaTypeForAzureFormat(format: string) {
  if (format.startsWith("riff") || format.startsWith("raw")) return format.startsWith("riff") ? "audio/wav" : "audio/pcm";
