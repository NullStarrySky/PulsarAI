...
export async function loadElevenLabsTtsSettings(): Promise<ElevenLabsTtsSettings> {
  ...
  const stored = await getDefaultConfig<Partial<ElevenLabsTtsSettings>>(settingsKey, fallback);
  return {
    ...
    similarityBoost: numberValue(stored.similarityBoost, fallback.similarityBoost, 0, 1),
