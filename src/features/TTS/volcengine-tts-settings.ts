...
export async function loadVolcengineTtsSettings(): Promise<VolcengineTtsSettings> {
	...
	const stored = await getDefaultConfig<Partial<VolcengineTtsSettings>>(
		settingsKey,
		fallback,
	);
	...
	return {
		...
		resourceId:
			typeof stored.resourceId === "string"
				? stored.resourceId.trim()
				: fallback.resourceId,
		speakerId:
			typeof stored.speakerId === "string"
				? stored.speakerId.trim()
				: fallback.speakerId,
		sampleRate:
