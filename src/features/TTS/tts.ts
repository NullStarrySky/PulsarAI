...
export function isVolcengineTtsModelRef(model: unknown): model is string {
  return typeof model === "string" && model.startsWith(`${VOLCENGINE_TTS_PROVIDER_ID}/`);
}

export const ELEVENLABS_TTS_PROVIDER_ID = "elevenlabs";
export const ELEVENLABS_TTS_API_KEY_SECRET = "elevenlabs_API_KEY";
export const ELEVENLABS_TTS_DEFAULT_URL = "https://api.elevenlabs.io";
...
export function isElevenLabsTtsModelRef(model: unknown): model is string {
  return typeof model === "string" && model.startsWith(`${ELEVENLABS_TTS_PROVIDER_ID}/`);
}
