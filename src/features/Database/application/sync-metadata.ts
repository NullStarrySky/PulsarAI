...
export function markLocalDatabaseChange(
  ...
  const record = value && typeof value === "object"
    ...
    : {};
  const scopePackageId = table === "resource_packages"
    ? id
    : typeof record.packageId === "string" || record.packageId === null
      ? record.packageId as string | null
      : previous?.scopePackageId;
  const parentConversationId = typeof record.conversationid === "string"
  ...
  snapshot.entities[key] = {
    ...
    syncable: table === "resource_packages"
...
export function mergeEntitySyncMeta(
  ...
  return {
    ...
    parentConversationId: local?.parentConversationId ?? remote?.parentConversationId,
    syncable: local?.syncable ?? remote?.syncable,
  };
}

export type VectorRelation = "equal" | "local-newer" | "remote-newer" | "concurrent";
