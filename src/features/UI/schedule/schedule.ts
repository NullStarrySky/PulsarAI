...
export function describeSchedule(task: ScheduleTask) {
	...
	const time =
		task.startTime === task.endTime
			? task.startTime
			: `${task.startTime}-${task.endTime} 随机`;
...
export function calculateNextRun(
	task: Pick<ScheduleTask, "period" | "startTime" | "endTime">,
	from = new Date(),
) {
	for (let offset = 0; offset < 14; offset += 1) {
		...
		const allowed =
			task.period.type === "daily" || task.period.weekdays.includes(weekday);
		if (!allowed) {
			continue;
		}
