...
`presentation/AppIcon.vue` is the shared application-identity image used by feature surfaces such as About.
...
`search/presentation/CommandSearchDialog.vue` composes commands, character packages, and conversations from their owning stores. Selecting a package or conversation updates the focused conversation surface directly; the palette does not open resource tabs.
