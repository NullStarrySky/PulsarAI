...
import {
	...
	createImportedFont,
	type FontDefinition,
} from "@/features/UI/font/font-registry";
import {
	builtInThemes,
...
export const useAppearanceStore = defineStore("appearance", () => {
	...
	const activeTheme = computed(
		() =>
			themes.value.find((theme) => theme.id === themeId.value) ??
			builtInThemes[0],
	);
	const activeFont = computed(
		() =>
			fonts.value.find((font) => font.id === fontId.value) ?? builtInFonts[0],
	);
	...
	function importThemeCss(css: string) {
		...
		customThemes.value = [
			...customThemes.value.filter((item) => item.id !== theme.id),
			theme,
		];
	...
	function applyAppearance() {
		...
		zenFrameIsDark.value = isCssColorDark(
			zenFrameEnabled.value ? "var(--zen-frame-bg)" : "var(--background)",
			topBarIsDark,
		);
		document.documentElement.style.setProperty(
			"--editor-font-size",
		...
		document.documentElement.style.setProperty(
			"--editor-line-height",
			`${editorLineHeight.value}px`,
		);
		void syncMobileNavigationBar(mobileNavigationBarMode.value, topBarIsDark);
	}
...
function installCustomThemeStyles(themes: ThemeDefinition[]) {
	...
	style.textContent = [...builtInThemes, ...themes]
		.map((theme) => theme.css ?? "")
		.join("\n\n");
}
...
function installSystemModeListener(
	mode: ThemeMode,
	onSystemChange: () => void,
) {
...
function readSnapshot(): AppearanceSnapshot {
	...
	try {
		...
		return {
			...
			agentLoadingStyle:
				parsed.agentLoadingStyle === "dots" ||
				parsed.agentLoadingStyle === "orbit"
			...
			frameCustomColor:
				typeof parsed.frameCustomColor === "string"
					? parsed.frameCustomColor
					: fallback.frameCustomColor,
			editorFontSize:
