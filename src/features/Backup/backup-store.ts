...
import { worldTable } from "@/features/Plugin/tree/world-persistence";
import {
	forgetPackageWorld,
	initializeWorlds,
} from "@/features/Plugin/tree/world-store";
...
type RestorableResourceType = "package" | "conversation";
...
async function syncableSnapshot(
	...
	const worlds = (await selectAll<WorldDocument>(worldTable))
		...
		.filter(
			(world) =>
				world.id === "global" ||
				packageIds.has(world.id.replace(/^package:/, "")),
		);
	...
	const entities = [
		...
		...worlds.map((value) => ({ table: worldTable, id: value.id, value })),
	];
	...
	const includedKeys = new Set([
		...
		...worlds.map((item) => syncEntityKey(worldTable, item.id)),
	]);
...
async function persistMergedSnapshot(
	...
	const localWorlds = new Map(
		(await selectAll<WorldDocument>(worldTable)).map((record) => [
			record.value.id,
			record.value,
		]),
	);
	...
	await withRemoteDatabaseWrites(async () => {
		for (const [key, remoteMeta] of Object.entries(remote.metadata)) {
			...
			if (
				...
				if (id.startsWith("package:"))
					forgetPackageWorld(id.slice("package:".length));
			}
		...
		for (const remoteWorld of remote.worlds) {
			const packageId = remoteWorld.id.startsWith("package:")
				? remoteWorld.id.slice("package:".length)
				: null;
			if (
				packageId &&
			...
			const local = localWorlds.get(remoteWorld.id);
			const key = syncEntityKey(worldTable, remoteWorld.id);
			const relation = entityRelation(
				local,
				remoteWorld,
				metadata.entities[key],
			...
			if (!local) {
				await upsert(worldTable, remoteWorld.id, clonePlain(remoteWorld));
				localWorlds.set(remoteWorld.id, clonePlain(remoteWorld));
				copied += 1;
			} else if (relation === "remote-newer") {
				await upsert(worldTable, remoteWorld.id, clonePlain(remoteWorld));
				localWorlds.set(remoteWorld.id, clonePlain(remoteWorld));
				...
				relation === "concurrent" &&
				!valuesEqual(local, remoteWorld)
			) {
				if (remoteWorld.updateDate >= local.updateDate) {
					await upsert(worldTable, remoteWorld.id, clonePlain(remoteWorld));
					localWorlds.set(remoteWorld.id, clonePlain(remoteWorld));
				}
				merged += 1;
...
export const useBackupStore = defineStore("backup", {
	...
	actions: {
		...
		async exportResource(key: string) {
			...
			if (!id || !["package", "conversation"].includes(type)) {
			...
			const payload: ResourceArchivePayload = {
				rootType: type,
				rootId: id,
				snapshot: { packages, conversations, containers, worlds },
			};
		...
		async importResourceArchive(mode: ResourceImportMode) {
			...
			if (!["package", "conversation"].includes(payload.rootType)) {
		...
		async restoreSelectedResources(
			...
			for (const sourceWorld of source.worlds) {
				if (!sourceWorld.id.startsWith("package:")) continue;
				const sourcePackageId = sourceWorld.id.slice("package:".length);
				const packageId = packageIdMap.get(sourcePackageId);
				if (!packageId) continue;
				const world = {
					...clonePlain(sourceWorld),
					id: `package:${packageId}`,
				};
				await upsert(worldTable, world.id, world);
			}
			await initializeWorlds();
			await Promise.all(
				[...packageIdMap.values()].map((packageId) =>
					initializeWorlds(packageId),
				),
			);
		...
		async updateSelectedResources(resourceArchivePath = "") {
			...
			for (const incoming of source.worlds) {
				if (!incoming.id.startsWith("package:")) continue;
				const packageId = incoming.id.slice("package:".length);
				if (!selectedPackageIds.has(packageId)) continue;
				const local = localWorlds.get(incoming.id);
				if (local) {
					resolvedDiffs += countDiffPaths(local, incoming).size;
					updated += 1;
				} else {
				...
				await upsert(worldTable, incoming.id, clonePlain(incoming));
			}
			await initializeWorlds();
			await Promise.all(
				[...selectedPackageIds].map((packageId) => initializeWorlds(packageId)),
			);
