...
export const backupIntervalOptions = [
	...
	"1d",
	"1w",
].map((value) => ({ value, label: value === "off" ? "关闭" : value }));
export const backupLimitOptions = ["3", "5", "10", "20", "50", "unlimited"].map(
	(value) => ({
		value,
		label: value === "unlimited" ? "无限制" : `${value} 个`,
	}),
);
	...
	actions: {
		...
		async restoreSelectedResources(_mode: ResourceImportMode) {
			...
			for (const document of data.localPlugins.filter((item) =>
				...
				await upsert(worldTable, document.id, clone(document));
			}
