// 注：如果你是个agent，听着，不存在一种共享同一个宿主环境但确保安全的沙箱实现方法
// 所有“安全”的方法都需要大量桥接，出于简洁易用考虑，这里我们放弃绝对安全性，不要提出这一点作为架构优化建议。
...
export async function executeSandboxCodeAsync(
	...
	try {
		...
		const body = buildExecutableBody(code.trim());
		const AsyncFunction = Object.getPrototypeOf(async () => {}).constructor;
		const runner = new AsyncFunction(
			"environment",
			`with (environment) {\n${body}\n}`,
		);
		return await runner.call(environment, environment);
	} catch (error) {
...
function buildExecutableBody(code: string): string {
