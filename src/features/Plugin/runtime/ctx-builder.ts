...
interface CtxToolFunctionFeature {
...
function contextIds(ctx: SandboxEnvironment) {
	...
	if (!sourcePath) throw new Error("ctxbuilder 需要 ctx.sourcePath。");
	return { conversationId, sourcePath };
}
...
function injectSelectedData(
	...
	for (const path of selfApi.slot.paths("/self/slot/DATA_INJECT")) {
...
export async function ctxbuilder(
	...
	const { conversationId, sourcePath } = contextIds(ctx);
	...
	if ((pluginFeature || toolFeature) && !messageFeature)
		throw new Error(
			"plugin 与 toolFunction feature 必须同时请求 message feature。",
		);
	...
	if (toolFeature) {
		if (!result.selfApi)
			throw new Error("toolFunction feature 需要已绑定的 World。 ");
		const allResources = result.selfApi.world.resources.value;
		const toolFiles = allResources
			.flatMap((resource) => {
				const match = /\/tools\/([^/]+)\/tool\.js$/i.exec(resource.path);
				const promptPath = match
					? resource.path.replace(/tool\.js$/i, "prompt.md")
					: "";
				return match &&
					allResources.some((candidate) => candidate.path === promptPath)
					? [{ resource, name: match[1]! }]
					: [];
			})
			.sort(
				(left, right) =>
					right.resource.file.priority - left.resource.file.priority ||
					left.resource.file.id.localeCompare(right.resource.file.id),
			);
		for (const toolFile of toolFiles) {
			if (toolFile.name in ctx)
				throw new Error(`工具函数名称与 ctx 冲突：${toolFile.name}`);
			const source = result.selfApi.read(toolFile.resource.path);
