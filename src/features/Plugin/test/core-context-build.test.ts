...
describe("Built-in Core context builder", () => {
  it("registers every non-panel Core pipeline resource through its slot", async () => {
    ...
    expect(nodes["context/before-regex.js"]?.insertion?.slot)
      .toBe("CTX_PROCESS_BEFORE_REGEX");
    expect(nodes["context/data.data.json"]?.insertion?.slot)
      .toBe("DATA_INJECT");
    expect(nodes["context/data.chat.json"]?.insertion?.slot)
      .toBe("data_prompt");
  });

  it("builds context through slots, inserts depth messages, and applies regex", async () => {
    const source = await readFile(resolve(corePath, "context/build.js"), "utf8");
    ...
    const result = await executeSandboxCodeAsync(source, [{
      ...
      imports: async (path: string, environment?: { messages?: unknown[] }) =>
