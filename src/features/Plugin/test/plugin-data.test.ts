...
describe("Plugin data definition", () => {
  it("keeps model-facing descriptions outside .data.json", () => {
    ...
    expect(parsePluginDataDefinition({ initialValue: { score: 1 } }))
      .toEqual(expect.objectContaining({ initialValue: { score: 1 } }));
    expect(parsePluginDataDefinition({ initialValue: {} }))
