...
)).flat().filter((rule) => rule && rule.applyOnRendering !== true);
...
messages = messages.map((message, index) => {
  ...
  for (const rule of rules) {
    ...
    const minimum = rule.depth_min === "INF" ? Infinity : Number(rule.depth_min);
    const maximum = rule.depth_max === "INF" ? Infinity : Number(rule.depth_max);
    if (!rangeMatches || depth < minimum || depth > maximum) continue;
    try {
      content = content.replace(new RegExp(rule.find_regex, "g"), rule.replace_regex);
    } catch {}
  }
