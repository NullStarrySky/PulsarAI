...
export type PluginDataValue = string | number | boolean | null | PluginDataValue[] | { [key: string]: PluginDataValue };

const valueSchema: z.ZodType<PluginDataValue> = z.lazy(() => z.union([
  z.string(), z.number().finite(), z.boolean(), z.null(),
  z.array(valueSchema), z.record(z.string(), valueSchema),
]));
...
export function parsePluginDataDefinition(input: unknown): PluginDataDefinition {
  let value = input;
...
export function createDataFacade(
  ...
  options: { readonly?: boolean; onReplace?: (value: PluginDataValue) => void } = {},
) {
  ...
  if (!facade || typeof facade !== "object") throw new Error(`${definition.name} 的 wrapper 必须返回对象。`);
  return options.readonly ? Object.freeze(facade) : facade;
}

function deepFreeze<T extends PluginDataValue>(value: T): T {
  if (!value || typeof value !== "object" || Object.isFrozen(value)) return value;
  Object.values(value).forEach((child) => deepFreeze(child as PluginDataValue));
