...
const rules = computed<RegexRule[]>(() => {
  try {
    const parsed = JSON.parse(props.modelValue) as unknown;
    return Array.isArray(parsed) ? parsed.filter((item): item is RegexRule => Boolean(item && typeof item === "object")) : [];
  } catch { return []; }
});
const emptyRule = (): RegexRule => ({ find_regex: "", replace_regex: "", range: "all", depth_min: 1, depth_max: "INF", applyOnRending: false });
function write(next: RegexRule[]) { emit("update:modelValue", JSON.stringify(next, null, 2)); }
function update(index: number, patch: Partial<RegexRule>) { const next = structuredClone(rules.value); next[index] = { ...next[index]!, ...patch }; write(next); }
