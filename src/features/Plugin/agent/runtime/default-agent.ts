...
const codeActInstructions = [
  ...
  "Inspect World containers with `slot.list()` / `get()`. `slot.paths(containerId)` returns enabled contribution paths after `/config.json.disabled` filtering; pass them to `await parse(...)` for recursive macro expansion. A chat resource returns pure message[] without authoring labels or disabled entries.",
  "World write/edit/mkdir/move/remove, writing `/config.json`, select(containerId, paths), and writable .data wrapper operations update the current message-bound World immediately. Disabled files or Plugin mounts stay directly readable but do not contribute to slots.",
  "World paths use `/self/path` for package-local files and `/global/<pluginId>/path` for global or built-in mounts. In Plugin source, `@/path` remains local to that source Plugin. Use open(path), close(path), or toggle(path) to operate the unified asset panel or a resource editor.",
