...
export async function generateImage(
	...
	if (isAutomatic1111ModelReference(model)) {
		if (model !== AUTOMATIC1111_MODEL_REF || typeof options.prompt !== "string")
			throw new Error("A1111 当前只支持 automatic1111/txt2img 文本生图。");
		...
		const result = await generateAutomatic1111Images({
			...
			useAuth: await hasAutomatic1111Auth(),
		});
		return specializedResult(result.images, {
			automatic1111: { images: [], seed: result.seed },
		});
	}
	if (isComfyUIModelReference(model)) {
		...
		return {
			...
			responses: [],
			providerMetadata: {
				comfyui: { images: [], promptId: result.promptId, seed: result.seed },
			},
			usage: {
	...
	if (isStabilityModelReference(model)) {
		if (typeof options.prompt !== "string")
			throw new Error("Stability 专用服务当前只接受文本提示词。");
		const modelId = model.slice(model.indexOf("/") + 1) as StabilityModelId;
		if (!STABILITY_MODELS.some((candidate) => candidate.id === modelId))
			throw new Error(`未知的 Stability 图片模型：${modelId}`);
		...
		return specializedResult(result.images, {
			stability: { images: [], model: modelId },
		});
	}
