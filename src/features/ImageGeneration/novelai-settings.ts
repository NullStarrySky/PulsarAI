...
export async function loadNovelAISettings(): Promise<NovelAISettings> {
	...
	const stored = await getDefaultConfig<Partial<NovelAISettings>>(
		settingsKey,
		fallback,
	);
	const settings = { ...fallback, ...stored };
	if (!NOVELAI_MODELS.some((model) => model.id === settings.model))
		settings.model = fallback.model;
	if (!NOVELAI_SAMPLERS.some((sampler) => sampler.id === settings.sampler))
		settings.sampler = fallback.sampler;
	settings.enabled = settings.enabled === true;
	settings.baseUrl =
		typeof settings.baseUrl === "string" ? settings.baseUrl : fallback.baseUrl;
	settings.width = normalizeNumber(
		settings.width,
		fallback.width,
		64,
		2048,
	...
	settings.guidance = normalizeNumber(
		settings.guidance,
		fallback.guidance,
		0,
		10,
	...
	settings.seed =
		settings.seed == null
			? null
			: normalizeNumber(settings.seed, 0, 0, 4294967295);
