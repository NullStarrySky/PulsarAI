...
const NOVELAI_DEFAULT_URL = "https://image.novelai.net";
...
type NovelAISamplerId = (typeof NOVELAI_SAMPLERS)[number]["id"];
...
export function isNovelAIModelReference(model: unknown): model is string {
	return (
		typeof model === "string" && model.startsWith(`${NOVELAI_PROVIDER_ID}/`)
	);
}
...
type ComfyUIWorkflowMode = "basic" | "custom";
...
export function isComfyUIModelReference(model: unknown): model is string {
	return (
		typeof model === "string" && model.startsWith(`${COMFYUI_PROVIDER_ID}/`)
	);
}
...
export function isStabilityModelReference(model: unknown): model is string {
	return (
		typeof model === "string" && model.startsWith(`${STABILITY_PROVIDER_ID}/`)
	);
}
