...
import { COMFYUI_RUNPOD_API_KEY_NAME, type ComfyUISettings } from "../domain/image-generation";
...
export async function testComfyUIConnection(settings: ComfyUISettings) {
  ...
  try {
    const response = await proxyFetch(`${baseUrl}/object_info/CheckpointLoaderSimple`, undefined, controller.signal);
    ...
    if (controller.signal.aborted) throw new Error(`连接 ${baseUrl} 超过 10 秒，请检查主机、端口与 ComfyUI 监听地址。`);
    throw error;
  } finally {
...
export async function generateComfyUIImages(options: ComfyUIGenerateOptions) {
  if (options.settings.serverType === "runpod") return generateRunPodImages(options);
  const baseUrl = buildComfyUIBaseUrl(options.settings);
  const seed = normalizeSeed(options.seed);
  const { workflow, outputNodeIds } = buildWorkflow(options.settings, options.prompt.trim(), seed, options.count ?? 1);
  if (!options.prompt.trim()) throw new Error("ComfyUI 图片提示词不能为空。");

  const controller = new AbortController();
  const timeout = window.setTimeout(() => controller.abort(), options.settings.timeoutSeconds * 1000);
  const forwardAbort = () => controller.abort();
  options.signal?.addEventListener("abort", forwardAbort, { once: true });
  try {
    const promptId = await queuePrompt(baseUrl, workflow, controller.signal);
    const images = await waitForImages(baseUrl, promptId, outputNodeIds, controller.signal);
    return { images: await downloadImages(baseUrl, images, controller.signal), seed, promptId };
  } catch (error) {
    if (controller.signal.aborted) {
      void modelProxyFetch(`${baseUrl}/interrupt`, { method: "POST" }).catch(() => undefined);
      if (options.signal?.aborted) throw new Error("已取消 ComfyUI 生成。");
      throw new Error(`ComfyUI 生成超过 ${options.settings.timeoutSeconds} 秒。`);
    }
    throw error;
  } finally {
    window.clearTimeout(timeout);
...
function buildRunPodBaseUrl(settings: ComfyUISettings) {
  ...
  if (!/^https?:\/\//i.test(url)) throw new Error("请填写完整的 RunPod Endpoint URL，例如 https://api.runpod.ai/v2/endpoint-id。");
  return url;
}

async function testRunPodConnection(settings: ComfyUISettings) {
  const baseUrl = buildRunPodBaseUrl(settings);
  const response = await modelProxyFetch(`${baseUrl}/health`, { headers: runPodHeaders() });
...
async function generateRunPodImages(options: ComfyUIGenerateOptions) {
  ...
  const seed = normalizeSeed(options.seed);
  const { workflow } = buildWorkflow(options.settings, options.prompt.trim(), seed, options.count ?? 1);
  ...
  try {
    while (!controller.signal.aborted) {
      const statusResponse = await modelProxyFetch(`${baseUrl}/status/${encodeURIComponent(jobId)}`, {
        headers: runPodHeaders(), signal: controller.signal,
      });
      const status = await readJson(statusResponse, "RunPod /status") as Record<string, unknown>;
      ...
      if (["FAILED", "CANCELLED", "TIMED_OUT"].includes(state)) throw new Error(`RunPod 任务失败：${state}`);
      await wait(500, controller.signal);
    }
    throw new Error("RunPod 生成已取消。");
  } catch (error) {
    if (controller.signal.aborted) void modelProxyFetch(`${baseUrl}/cancel/${encodeURIComponent(jobId)}`, { method: "POST", headers: runPodHeaders() });
    throw error;
  } finally {
    window.clearTimeout(timeout);
    options.signal?.removeEventListener("abort", forwardAbort);
...
function readRunPodImages(payload: Record<string, unknown>): NovelAIGeneratedImage[] {
  if (!isRecord(payload.output) || !Array.isArray(payload.output.images)) return [];
  return payload.output.images.flatMap((item) => {
    if (!isRecord(item) || typeof item.data !== "string") return [];
    const normalized = item.data.includes(",") ? item.data.slice(item.data.indexOf(",") + 1) : item.data;
    const binary = atob(normalized);
    const bytes = Uint8Array.from(binary, (character) => character.charCodeAt(0));
    const filename = typeof item.filename === "string" ? item.filename : "image.png";
    return [{ base64: normalized, mediaType: mediaTypeForName(filename), uint8Array: bytes }];
  });
}

export function buildComfyUIBaseUrl(settings: Pick<ComfyUISettings, "protocol" | "host" | "port">) {
...
function buildWorkflow(settings: ComfyUISettings, prompt: string, seed: number, count: number) {
  if (settings.workflowMode === "basic") {
    if (!settings.checkpoint.trim()) throw new Error("请先连接 ComfyUI 并选择 checkpoint。");
    return { workflow: createBasicWorkflow(settings, prompt, seed, count), outputNodeIds: ["9"] };
  }
  const workflow = parseCustomWorkflow(settings.workflowJson);
  applyCustomBindings(workflow, { prompt, negativePrompt: settings.negativePrompt, seed, width: settings.width, height: settings.height });
  const outputNodeIds = findOutputNodeIds(workflow);
  if (!outputNodeIds.length) throw new Error("自定义工作流没有 SaveImage、PreviewImage 或 imageOutput 标记节点。");
  stripPulsarMetadata(workflow);
  return { workflow, outputNodeIds };
}

function createBasicWorkflow(settings: ComfyUISettings, prompt: string, seed: number, count: number): Workflow {
  return {
    "3": { class_type: "KSampler", inputs: { seed, steps: settings.steps, cfg: settings.cfg, sampler_name: settings.sampler, scheduler: settings.scheduler, denoise: 1, model: ["4", 0], positive: ["6", 0], negative: ["7", 0], latent_image: ["5", 0] } },
    "4": { class_type: "CheckpointLoaderSimple", inputs: { ckpt_name: settings.checkpoint } },
    "5": { class_type: "EmptyLatentImage", inputs: { width: settings.width, height: settings.height, batch_size: Math.min(4, Math.max(1, Math.trunc(count))) } },
    "6": { class_type: "CLIPTextEncode", inputs: { text: prompt, clip: ["4", 1] } },
    "7": { class_type: "CLIPTextEncode", inputs: { text: settings.negativePrompt, clip: ["4", 1] } },
    "8": { class_type: "VAEDecode", inputs: { samples: ["3", 0], vae: ["4", 2] } },
    "9": { class_type: "SaveImage", inputs: { filename_prefix: "PulsarAI", images: ["8", 0] } },
  };
}

function parseCustomWorkflow(source: string): Workflow {
  if (!source.trim()) throw new Error("请粘贴 ComfyUI API-format 工作流 JSON。");
  let parsed: unknown;
  try {
  ...
  if (!isRecord(parsed) || !Object.keys(parsed).length) throw new Error("ComfyUI 工作流必须是非空对象。");
...
function applyCustomBindings(workflow: Workflow, values: Record<string, string | number>) {
  for (const node of Object.values(workflow)) {
    const metadata = node._meta?.pulsar ?? node._meta?.cosmosVision;
    for (const [inputName, binding] of Object.entries(metadata?.promptBindings ?? {})) {
      node.inputs[inputName] = binding === "positive" ? values.prompt : values.negativePrompt;
    }
    for (const [inputName, value] of Object.entries(node.inputs)) {
      if (typeof value !== "string") continue;
      const exact = value.match(/^\{\{(prompt|negativePrompt|seed|width|height)\}\}$/);
      if (exact) {
        node.inputs[inputName] = values[exact[1]!]!;
        continue;
      }
      node.inputs[inputName] = value
        .split("{{prompt}}").join(String(values.prompt))
        .split("{{negativePrompt}}").join(String(values.negativePrompt));
    }
  }
}
...
function findOutputNodeIds(workflow: Workflow) {
  const marked = Object.entries(workflow)
    .filter(([, node]) => node._meta?.pulsar?.imageOutput || node._meta?.cosmosVision?.imageOutput)
    .map(([nodeId]) => nodeId);
...
async function queuePrompt(baseUrl: string, workflow: Workflow, signal: AbortSignal) {
  const response = await proxyFetch(`${baseUrl}/prompt`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ client_id: `pulsarai-${crypto.randomUUID()}`, prompt: workflow }),
  }, signal);
  const payload = await readJson(response, "ComfyUI /prompt") as { prompt_id?: string };
  if (!payload.prompt_id) throw new Error("ComfyUI /prompt 未返回 prompt_id。");
  return payload.prompt_id;
}

async function waitForImages(baseUrl: string, promptId: string, outputNodeIds: string[], signal: AbortSignal) {
  while (!signal.aborted) {
    const response = await proxyFetch(`${baseUrl}/history/${encodeURIComponent(promptId)}`, undefined, signal);
...
function readHistoryEntry(payload: unknown, promptId: string): HistoryEntry | null {
  if (!isRecord(payload)) return null;
  if (isRecord(payload.outputs)) return payload as HistoryEntry;
  return isRecord(payload[promptId]) ? payload[promptId] as HistoryEntry : null;
}

function readHistoryImages(entry: HistoryEntry | null, outputNodeIds: string[]): HistoryImage[] | null {
  if (!entry?.outputs) return null;
  const images = outputNodeIds.flatMap((nodeId) => entry.outputs?.[nodeId]?.images ?? []).filter((image) => image.filename);
...
async function downloadImages(baseUrl: string, images: HistoryImage[], signal: AbortSignal): Promise<NovelAIGeneratedImage[]> {
  const results: NovelAIGeneratedImage[] = [];
  for (const image of images) {
    const query = new URLSearchParams({ filename: image.filename, subfolder: image.subfolder ?? "", type: image.type ?? "output" });
    const response = await proxyFetch(`${baseUrl}/view?${query}`, undefined, signal);
    if (!response.ok) throw new Error(`ComfyUI /view 请求失败 (${response.status})。`);
    const bytes = new Uint8Array(await response.arrayBuffer());
    results.push(binaryImage(bytes, response.headers.get("content-type") || mediaTypeForName(image.filename)));
  }
  return results;
}
...
function extractCheckpointNames(payload: unknown) {
  if (!isRecord(payload) || !isRecord(payload.CheckpointLoaderSimple)) return [];
  ...
  return value[0].filter((item): item is string => typeof item === "string" && Boolean(item.trim()));
}
...
function normalizeSeed(seed?: number) {
  if (seed == null || !Number.isFinite(seed)) return Math.floor(Math.random() * 4294967296);
...
function proxyFetch(input: RequestInfo | URL, init?: RequestInit, signal?: AbortSignal) {
