...
import { STABILITY_API_KEY_NAME, type StabilityModelId, type StabilitySettings } from "../domain/image-generation";
...
export async function generateStabilityImages(options: {
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    if (options.settings.negativePrompt.trim()) form.set("negative_prompt", options.settings.negativePrompt.trim().slice(0, 10000));
    form.set("aspect_ratio", options.settings.aspectRatio);
    form.set("output_format", options.settings.outputFormat);
    if (options.settings.stylePreset) form.set("style_preset", options.settings.stylePreset);
    if (options.seed != null && Number.isFinite(options.seed)) form.set("seed", String(Math.max(0, Math.trunc(options.seed)) + index));
    if (options.model === "stable-diffusion-3") form.set("model", "sd3.5-large");

    const baseUrl = options.settings.baseUrl.trim().replace(/\/+$/, "");
    const response = await modelProxyFetch(`${baseUrl}${paths[options.model]}`, {
      method: "POST",
      headers: { Authorization: `Bearer <<${STABILITY_API_KEY_NAME}>>`, Accept: "image/*" },
      body: form,
      signal: options.signal,
    });
    if (!response.ok) throw new Error(`Stability 请求失败 (${response.status})：${(await response.text()).slice(0, 300)}`);
    const bytes = new Uint8Array(await response.arrayBuffer());
    const mediaType = response.headers.get("content-type") || `image/${options.settings.outputFormat}`;
