	...
	if (systemSupported) {
		try {
			...
			await startSystemStt({
				language:
					defaults.sttLanguage !== "auto" ? defaults.sttLanguage : undefined,
				maxDuration: 60_000,
				onDevice: true,
			});
		} catch (error) {
			push.error(
				error instanceof Error ? error.message : "启动系统语音识别失败",
			);
			return;
		}
	} else {
...
async function processResultText(rawText: string) {
	...
	if (defaults.sttAutoPolish) {
		...
		try {
			const polishModel =
				defaults.sttPolishModel ||
				defaults.fastModel ||
				defaults.defaultChatModel;
			const promptTemplate =
				defaults.sttPolishPrompt ||
