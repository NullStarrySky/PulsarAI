...
describe("Sandbox", () => {
	it("evaluates async environment capabilities instead of module imports", async () => {
		const value = await executeSandboxCodeAsync(
			...
			[
				{
					imports: async (path: string) =>
						path === "./context.md" ? "resolved context" : null,
				},
			],
		);
	...
	it("recursively resolves async imports in text", async () => {
		...
		const imports = async (path: string) =>
			resolveSandboxTextAsync(resources[path]!, [{ name: "Pulsar", imports }]);

		await expect(
			resolveSandboxTextAsync('root {{ await imports("./first.md") }}', [
				{ imports },
			]),
	...
	it("splices recursively imported chat messages into the surrounding context", async () => {
		const imports = async () => [
			{ role: "system" as const, content: "nested {{ name }}" },
		];
		await expect(
			resolveSandboxMessagesAsync(
				[
					...
					{
						role: "system",
						content: '[[ await imports("./nested.chat.json") ]]',
					},
