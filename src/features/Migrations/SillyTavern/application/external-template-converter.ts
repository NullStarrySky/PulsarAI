...
const unsupportedEjsRuntime = new Map<string, string>([
  ...
  ["include", "EJS include 没有可验证的模板根路径；应改用 imports.resource() 显式引用。"],
  ["SillyTavern", "不向 PulsarAI 执行层注入 SillyTavern API 兼容对象。"],
  ["TavernHelper", "不向 PulsarAI 执行层注入 TavernHelper API。"],
  ["getwi", "世界书读取必须改用来源作用域内的 imports.resource()/container()。"],
  ["getWorldInfo", "世界书读取必须改用来源作用域内的 imports.resource()/container()。"],
...
function macroExpression(nameInput: string, args: string[], context: ExternalTemplateContext) {
  ...
  switch (name) {
    ...
    case "model": return `String(imports.config.local("generation", "model") ?? "")`;
