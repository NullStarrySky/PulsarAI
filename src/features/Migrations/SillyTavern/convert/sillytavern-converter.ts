...
function convertQuickReplies(snapshot: SillyTavernSourceSnapshot): QuickReplyMigrationArtifact[] {
  ...
  if (snapshot.settings) {
    ...
    if (quickReplySettings) {
      artifacts.push(...convertQuickReplyValue(
        quickReplySettings,
        settingsSource ?? {
          path: "settings.json",
          relativePath: "settings.json",
  ...
  for (const resource of snapshot.resources) {
    if (resource.discrimination.kind !== "quick-reply" || !isRecord(resource.value)) continue;
    artifacts.push(...convertQuickReplyValue(resource.value, resource.source, resource.entry.name));
  }
  return artifacts;
}
...
function convertQuickReplyValue(
  ...
  return quickReplySets(value, fallbackSetName).flatMap(({ name: setName, replies }) =>
    replies.flatMap((reply, index): QuickReplyMigrationArtifact[] => {
      if (!isRecord(reply)) return [];
      const diagnostics: MigrationDiagnostic[] = [];
      const content = stringValue(reply.mes ?? reply.message ?? reply.content);
      const name = stringValue(reply.label ?? reply.title ?? reply.name) || `快速回复 ${index + 1}`;
      if (!content.trim()) {
        diagnostics.push(migrationDiagnostic(
      ...
      return [{
        ...
        diagnostics,
        unconsumedFields: unconsumedKeys(reply, ["id", "label", "title", "name", "mes", "message", "content", "hidden"]),
        name,
        content: convertExternalText(content, source, diagnostics, templateContext, `qrList.${index}.mes`),
        setName,
      }];
    }),
  );
...
function quickReplySets(value: Record<string, unknown>, fallbackName: string) {
  ...
  return candidates.flatMap((candidate, index) => {
    ...
    return [{
      name: stringValue(candidate.name ?? candidate.title) || (index ? `${fallbackName}-${index + 1}` : fallbackName),
      replies,
    }];
  });
}
