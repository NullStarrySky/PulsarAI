...
import { buildStImportPlan, readStResourceFile, tryBuildStImportPlan, type StImportPlan, type StResourceFile } from "./st-import-plan";
...
const error = ref("");
const applied = ref("");
const raw = computed(() => source.value?.text ?? (source.value?.base64 ? `[${source.value.mediaType ?? "binary"}]` : ""));
async function choose() {
	const selected = await host.dialog.open({ title: "选择 SillyTavern 资源", multiple: false, directory: false, properties: ["openFile"], filters: [{ name: "SillyTavern 资源", extensions: ["png", "json", "md", "markdown", "txt"] }] });
	if (typeof selected !== "string") return;
	error.value = ""; applied.value = ""; source.value = await readStResourceFile(selected); plan.value = null;
}
function convert() {
	if (!source.value) return;
	error.value = ""; applied.value = "";
	try { plan.value = tryBuildStImportPlan(source.value.fileName, source.value) ?? buildStImportPlan(source.value.fileName, source.value); }
	catch (cause) { plan.value = null; error.value = cause instanceof Error ? cause.message : String(cause); }
}
async function apply() {
	if (!plan.value) return;
	try { const result = await applyStImportPlan(world, plan.value, "/self"); applied.value = `已写入测试 World：${result.createdPaths.length} 个节点`; }
...
<template>
  <div class="grid h-full min-h-0 gap-3 p-3 md:grid-cols-[minmax(0,1fr)_13rem_minmax(0,1fr)]">
    <section class="min-h-0 rounded-xl border bg-card p-3"><div class="mb-3 flex items-center justify-between"><h2 class="font-medium">原始资源</h2><Button size="sm" @click="choose"><Upload class="mr-1 size-4" />上传</Button></div><p class="mb-2 text-xs text-muted-foreground">{{ source?.fileName ?? "选择 PNG、JSON 或 Markdown 文件" }}</p><ScrollArea class="h-[calc(100%-4rem)] rounded-lg bg-muted/30 p-3"><pre class="whitespace-pre-wrap break-words text-xs">{{ raw }}</pre></ScrollArea></section>
