...
import { createPluginMediaContent } from "@/features/Resources/Plugin/editors/media/plugin-media";
import {
  pluginFileType,
  findPluginTreeNode,
...
type NewPluginFileType = "agents" | "skill" | "markdown" | "chat" | "data" | "javascript" | "json" | "media" | "component" | "text";
type ClipboardNode = {
  name: string;
  kind: "file" | "folder";
  content?: unknown;
...
const newFileTypes: Array<{ id: NewPluginFileType; label: string; extension: string }> = [
  ...
  { id: "skill", label: "Skill", extension: ".md" },
...
function newFileTemplate(type: NewPluginFileType) {
  ...
  if (type === "skill") return { name: "untitled-skill.md", content: "# Skill\n\n描述此 Skill 的用途、边界与使用步骤。\n" };
  if (type === "data") return {
