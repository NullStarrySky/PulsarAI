...
} from "@/features/Resources/Plugin/tree/plugin-persistence";
import {
...
} from "@/features/Resources/Plugin/runtime/plugin-reference";
import {
  manifestValueAt,
...
import { createBuiltinPlugins } from "@/features/Resources/Plugin/tree/builtin-plugins";
import { emitPluginConfigChange } from "@/features/Resources/Plugin/editors/manifest/plugin-config-events";
import { createPluginReferenceResolver } from "@/features/Resources/Plugin/runtime/plugin-reference-resolver";
import { backgroundPathSelectionOptions } from "@/features/Resources/Plugin/tree/plugin-path-selection";
        ...
        [
          createFile(
            ...
            [
              ...
              "Call read_docs(featureId?, apiName?) before using an unfamiliar Feature API. It returns the Feature directory, a Feature definition, or one API contract (or null); blocked entries are not callable.",
  ...
  actions: {
    ...
    async renamePluginId(pluginId: string, requestedId: string) {
      ...
      const { useConversationStore } =
        await import("@/features/Resources/Conversation/store/conversation-store");
