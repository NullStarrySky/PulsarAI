...
import { pluginFileMatchesContainerSuffix } from "../domain/plugin-reference";
import { pluginMediaSource, pluginMediaType } from "../domain/plugin-media";
import { pluginConventions, pluginFileType, type Plugin, type PluginFile } from "../domain/plugin-types";
import PluginFileEditorSurface from "./PluginFileEditorSurface.vue";
import PluginInsertionConditionEditor from "./PluginInsertionConditionEditor.vue";
...
function updateInsertion(value: unknown) {
  ...
  if (insertionTarget.value === "none") insertionCondition.value = "";
  scheduleSave();
}
          ...
          <header class="plugin-file-drag-handle flex min-h-16 shrink-0 cursor-move flex-wrap items-center gap-2 border-b border-border/80 px-4 py-2 mobile:cursor-default">
            ...
            <div class="plugin-file-control contents">
              <Select :model-value="insertionTarget" @update:model-value="updateInsertion">
                <SelectTrigger class="h-8 min-w-40 max-w-64 text-xs" aria-label="插入位置">
