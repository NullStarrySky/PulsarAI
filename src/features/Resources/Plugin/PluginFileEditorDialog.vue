...
import { createPluginSelfApi } from "./runtime/plugin-self-api";
import type { TraceLogEntry } from "@/features/Resources/Plugin/runtime/plugin-test";
import { pluginFileMatchesContainerSuffix } from "@/features/Resources/Plugin/runtime/plugin-reference";
import { pluginMediaSource, pluginMediaType } from "@/features/Resources/Plugin/editors/media/plugin-media";
import type { PluginManifestValue } from "@/features/Resources/Plugin/editors/manifest/plugin-manifest";
import { pluginConventions, pluginFileType, type Plugin, type PluginFile } from "@/features/Resources/Plugin/tree/plugin-types";
import PluginFileEditorSurface from "@/features/Resources/Plugin/editors/PluginFileEditorSurface.vue";
import PluginInsertionConditionEditor from "@/features/Resources/Plugin/shared/PluginInsertionConditionEditor.vue";
import PluginParseResultTree from "@/features/Resources/Plugin/editors/PluginParseResultTree.vue";
...
const containerOptions = computed(() => {
  ...
  const plugins = visiblePlugins.value.some((item) => item.id === plugin.id)
    ? visiblePlugins.value
    : [plugin, ...visiblePlugins.value];
  const resolver = createPluginReferenceResolver(plugins, {
    environment: { chat: [], CHAT: [], PROJECT_AGENT_PROMPT: "[PROJECT_AGENT_PROMPT]" },
  });
  return resolver.listContainers().flatMap((summary) => {
    const container = resolver.getContainer(summary.id);
    if (!container || !pluginFileMatchesContainerSuffix(file.name, container.contentSuffixes)) return [];
    if (container.scope !== "global" && container.pluginId !== plugin.id) return [];
    return [{
      value: `container:${container.scope}/${container.name}`,
