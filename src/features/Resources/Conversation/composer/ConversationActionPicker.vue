...
import { FileText, PanelsTopLeft, Play, X } from "lucide-vue-next";
import { Button } from "@/components/ui/button";
import type { ActionPart } from "@/features/Resources/Conversation/messages/conversation-types";
...
const emit = defineEmits<{
  ...
  "open-view": [action: ResolvedPluginAction];
}>();
    ...
    >
      ...
      <div class="max-h-64 overflow-y-auto p-1.5">
        ...
        >
          ...
          />
          <Play v-else-if="action.kind === 'process'" class="mt-0.5 size-3.5 text-muted-foreground" />
          <PanelsTopLeft v-else class="mt-0.5 size-3.5 text-muted-foreground" />
          <span class="min-w-0">
            <span class="block truncate font-mono text-sm font-medium">/{{ action.resource.name }}</span>
            <span class="mt-0.5 block truncate text-xs text-muted-foreground">
              {{ action.pluginName }} · {{ action.kind === "prompt" ? "填入输入框" : action.kind === "view" ? "打开视图" : "执行命令" }}
            </span>
          </span>
        </button>
