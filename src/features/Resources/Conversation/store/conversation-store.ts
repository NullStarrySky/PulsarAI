...
import {
  builtinBlankPluginId,
  builtinCorePluginId,
  usePluginStore,
} from "@/features/Resources/Plugin/tree/plugin-store";
...
import { buildConversationResourceContext } from "@/features/Resources/Conversation/store/conversation-resource-context";
import { deleteConversationMemory } from "@/features/Resources/Conversation/generation/conversation-memory";
import type {
  CharacterPackage,
  ActionPart,
  AdditionalParts,
...
import { formatChatMessageError } from "@/features/Resources/Conversation/messages/conversation-types";
...
export interface ConversationContainerRequest {
  mode: ConversationRequestMode;
  /** Internal callers may target another conversation; the public API stays on the active one. */
  conversationId?: string;
  /** Existing ordinary container required by regenerate, continue, and rewrite. */
  containerId?: string;
  /** Parent for a newly generated ordinary or command container. */
  previousContainerId?: string;
  /** Required only by command mode. */
  action?: ActionPart;
  prompt?: string;
}
...
function createMessage(content = "", type: ChatMessageType = "message"): ChatMessage {
  return {
    ...
    type,
    content: type === "error"
      ? formatChatMessageError(normalizeMarkdownLineBreaks(content))
...
export function createEmptyChatMessage(content = "", type: ChatMessageType = "message"): ChatMessage {
  return createMessage(content, type);
}
...
function cloneMessage(message: ChatMessage): ChatMessage {
  return {
    ...
    content: normalizeMarkdownLineBreaks(message.content),
    createdAt: now(),
...
export function modelMessagesFromPath(path: ChatMessageContainer[]): ModelMessage[] {
  ...
  for (const item of path) {
    ...
    const message = item.activeMessage === null
      ? null
      : item.content[item.activeMessage] ?? null;
    if (message?.type === "error") continue;
    const content = message ? normalizeMarkdownLineBreaks(message.content).trim() : "";
    const fileParts = message?.parts?.filter(
      (part) => part.type === "file" || part.type === "image",
    ) ?? [];
    if (!content && fileParts.length === 0) continue;
    if (item.role === "system") {
...
function createContainer(input: {
  ...
  hidden?: boolean;
  command?: ChatMessageContainer["command"];
  draft?: string;
}): ChatMessageContainer {
  ...
  return {
    ...
    ...(input.command ? { command: clonePlain(input.command) } : {}),
    ...(input.draft !== undefined ? { draft: normalizeMarkdownLineBreaks(input.draft) } : {}),
  };
}
...
export const useConversationStore = defineStore("conversation", {
  ...
  getters: {
    activePackage: (state) =>
      state.packages.find((item) => item.id === state.activePackageId),
  ...
  actions: {
    ...
    async loadInitialData() {
      const [packages, categories, conversations, containers] = await Promise.all([
      ...
      const nextPackages = packages.map(({ value }) => ({
        ...value,
        pluginId: value.pluginId ?? "",
        mainPluginId: value.mainPluginId || builtinCorePluginId,
        enabledGlobalPluginIds: value.enabledGlobalPluginIds ?? [],
    ...
    async ensurePackagePlugin(packageId: string) {
      ...
      if (pluginStore.loadError) {
        throw new Error(`插件数据读取失败：${pluginStore.loadError}`);
      }
      ...
      if (item.pluginId !== localPlugin.id) {
    ...
    async updatePackage(packageId: string, patch: Partial<Pick<CharacterPackage, "name" | "description" | "icon" | "categoryId" | "pinned" | "syncEnabled" | "mainPluginId" | "enabledGlobalPluginIds">>) {
      const item = this.packages.find((packageItem) => packageItem.id === packageId);
      if (!item) {
        return;
      }
      ...
      if (patch.mainPluginId || patch.enabledGlobalPluginIds) {
        ...
        if (patch.mainPluginId) {
          ...
          if (!pluginGenerateFile(mainPlugin)) {
            throw new Error("主要插件必须配置存在且有效的 runtime/generatePath。");
          }
        }
    ...
    cloneConversationContainers(templateConversationId: string, conversationId: string) {
      ...
      const containers = sourceContainers.map((source, index): ChatMessageContainer => {
        ...
        return {
          ...
          ...(source.command ? { command: clonePlain(source.command) } : {}),
    ...
    modelMessagesBefore(container: ChatMessageContainer): ModelMessage[] {
      return modelMessagesFromPath(this.containerPathTo(container.previousContainer));
    },
    ...
    async switchMessage(containerId: string, index: number) {
      const container = this.containers.find((item) => item.id === containerId);
      if (!container || container.hidden || index < 0 || index >= container.content.length) {
        return;
      }
    ...
    async annotateMessage(
      ...
      const directive = String(instruction ?? "").trim();
      if (!container || !message || !selected || !directive) return false;
      const index = message.content.indexOf(selected);
      if (index < 0) return false;
      const escaped = directive
        .replace(/&/g, "&amp;")
        .replace(/"/g, "&quot;")
        .replace(/</g, "&lt;");
      message.content = `${message.content.slice(0, index)}<pulsar-rewrite instruction="${escaped}">${selected}</pulsar-rewrite>${message.content.slice(index + selected.length)}`;
      await this.persistContainer(container);
      return true;
    },
    async setMessageTranslation(
      ...
      message.meta.translation ??= {
        originalContent: normalizeMarkdownLineBreaks(message.content),
        translatedAt: now(),
      };
      const normalizedContent = normalizeMarkdownLineBreaks(translatedContent);
      message.content = normalizedContent;
      await this.persistContainer(container);
      return true;
    },
    ...
    async appendContainerToConversation(
      ...
      options: Pick<ChatMessageContainer, "hidden" | "command" | "draft"> = {},
    ) {
    ...
    async requestContainer(
      input: ConversationContainerRequest,
      mutate?: MessageDraftClosure,
    ) {
      const mode = input?.mode;
      if (!mode) throw new Error("requestContainer 需要 mode。");
      if (!(["generate", "command", "regenerate", "continue", "rewrite"] as const).includes(mode)) {
        throw new Error(`不支持的容器请求 mode：${String(mode)}`);
      }
      if (mode === "generate" || mode === "command") {
        if (input.containerId) {
          throw new Error(`${mode} mode 只能创建新容器，不能指定既有 containerId。`);
        }
        const conversationId = input.conversationId ?? this.activeConversationId;
        if (!conversationId || this.generatingConversationIds.includes(conversationId)) return null;
        const conversation = this.conversations.find((item) => item.id === conversationId);
        if (!conversation) return null;
        if (mode === "command" && input.action?.type !== "action") {
          throw new Error("command mode 需要 ActionPart。");
        }
        const container = await this.appendContainerToConversation(
          conversation.id,
          "assistant",
          "",
          input.previousContainerId,
          mode === "command" ? [clonePlain(input.action!)] : undefined,
          true,
          "message",
          mode === "command"
            ? {
                ...
                command: {
                  ...
                  args: normalizeMarkdownLineBreaks(input.prompt ?? ""),
                },
                draft: "",
              }
            : {},
        ...
        await this.fillAssistantContainer(
          container,
          mutate,
          undefined,
          mode === "command"
            ? { action: input.action!, prompt: normalizeMarkdownLineBreaks(input.prompt ?? "") }
            : undefined,
        );
        return container;
      }

      if (input.action || input.previousContainerId) {
        throw new Error(`${mode} mode 只作用于既有容器，不能传入 action 或 previousContainerId。`);
      }
      const containerId = input.containerId ?? this.activeContainerId;
      const container = this.containers.find((item) => item.id === containerId);
      ...
      if (
        ...
        || this.generatingConversationIds.includes(container.conversationid)
      ) {
        return null;
      }
      ...
      const generation = mode === "rewrite"
        ? {
            prompt: [
              "请按下列助手回复中的 <pulsar-rewrite instruction=\"...\"> 标注重写内容。保留未标注部分的事实与格式；只输出重写后的干净 Markdown，不要保留或提及 pulsar-rewrite 标签。",
              "",
              "# 要重写的回复",
              message.content,
            ].join("\n"),
        ...
        : {
            prompt: [
              "请从下列助手回复的末尾自然继续。只输出新增的续写部分，不要重复已有内容。",
              "",
              "# 当前回复",
              message.content,
            ].join("\n"),
      ...
      await this.fillAssistantContainer(container, mutate, undefined, undefined, generation);
      return container;
    },
    async pushChatMessage(
      input: { role: Role; content: string; meta?: Partial<ChatMessageMeta> },
      merge = false,
      conversationId?: string,
    ) {
      const targetConversationId = conversationId ?? this.activeConversationId;
      if (!targetConversationId) return null;
      const content = normalizeMarkdownLineBreaks(input.content);
      const conversation = this.conversations.find((item) => item.id === targetConversationId);
      if (!conversation) return null;
      const tailId = conversation.lastContainerId
        ? this.resolveActiveTailId(conversation.lastContainerId)
        : null;
      const tail = tailId
        ? this.containers.find((item) => item.id === tailId)
        : null;
      if (merge && tail?.role === input.role) {
        const message = this.currentMessage(tail);
        if (!message) return null;
        message.content = normalizeMarkdownLineBreaks(
          message.content && content ? `${message.content}\n${content}` : message.content || content,
        );
        if (input.meta) {
          Object.assign(message.meta, clonePlain(input.meta), {
            steps: clonePlain(input.meta.steps ?? message.meta.steps),
          });
        }
        if (targetConversationId === this.activeConversationId) this.activeContainerId = tail.id;
        await this.persistContainer(tail);
        return tail;
      }
      const container = await this.appendContainerToConversation(
        targetConversationId,
    ...
    async send(
      ...
      const trimmed = normalizeMarkdownLineBreaks(content).trim();
      if (this.generatingConversationIds.includes(this.activeConversationId)) {
        return;
      }
      if (action) {
        await this.requestContainer({ mode: "command", action, prompt: trimmed }, mutate);
        return;
      }
      if (!trimmed) return;
      void import("@/features/Statistic/statistic-store").then(({ useStatisticStore }) =>
      ...
      await this.requestContainer({ mode: "generate" }, mutate);
    },
    async runSubAgent(
      request: GenerateSubAgentRequest,
      source: { conversation: Conversation; depth?: number },
    ): Promise<GenerateSubAgentResult> {
      const depth = source.depth ?? 0;
      if (depth >= 3) {
        throw new Error("子代理最多嵌套三层。");
      }
      const prompt = normalizeMarkdownLineBreaks(request.prompt).trim();
      if (!prompt) throw new Error("子代理 prompt 不能为空。");
      const contextConversation = request.environment
        ? this.conversations.find((item) => item.id === request.environment)
        : source.conversation;
      if (!contextConversation) {
        throw new Error(`子代理环境会话不存在：${request.environment}`);
      }
      const packageItem = this.packages.find(
        (item) => item.id === contextConversation.packageId,
      );
      if (!packageItem) throw new Error("子代理环境缺少角色包。");

      const ephemeral = !request.environment;
      ...
      const childConversation: Conversation = ephemeral
        ? {
            id: root.conversationid,
            packageId: contextConversation.packageId,
            kind: "chat",
            title: "临时子代理会话",
            isEphemeral: true,
            rendererId: "chat",
            rootContainerId: root.id,
            lastContainerId: root.id,
            createdAt: now(),
            updatedAt: now(),
          }
        : contextConversation;
      const contextPath = ephemeral
        ? [root]
        : this.containerPathForConversation(contextConversation);
      const previous = contextPath[contextPath.length - 1] ?? root;
      const promptContainer = createContainer({
        role: "user",
        conversationId: childConversation.id,
        previousContainer: previous.id,
        content: prompt,
      });
      const assistantContainer = createContainer({
        role: "assistant",
        conversationId: childConversation.id,
        previousContainer: promptContainer.id,
        content: "",
      });
      const assistantMessage = this.currentMessage(assistantContainer);
      if (!assistantMessage) throw new Error("无法创建子代理回复目标。");

      const pluginStore = usePluginStore();
      await pluginStore.initialize();
      const mainPluginId = request.plugin?.trim() || builtinBlankPluginId;
      const mainPlugin = pluginItems(pluginStore).find(
        (plugin) => plugin.id === mainPluginId,
      );
      if (!mainPlugin) throw new Error(`子代理插件不存在：${mainPluginId}`);
      if (mainPlugin.packageId && mainPlugin.packageId !== childConversation.packageId) {
        throw new Error("子代理只能使用当前角色包或全局插件。");
      }
      const plugins = pluginStore.enabledPluginsForPackage(
        childConversation.packageId,
        packageItem.enabledGlobalPluginIds,
        mainPluginId,
      );
      if (!plugins.some((plugin) => plugin.id === mainPlugin.id)) {
        plugins.unshift(mainPlugin);
      }
      const activePath = [...contextPath, promptContainer].filter(
    ...
    async sendToConversation(
      ...
      await this.requestContainer({
        mode: "generate",
        conversationId,
        previousContainerId: userContainer.id,
      }, mutate);
    },
    async fillAssistantContainer(
      ...
      command?: { action: ActionPart; prompt: string },
      generation?: {
        prompt: string;
      ...
      try {
        ...
        if (generation?.prompt.trim()) {
          generationPath.push({
            id: `generation-input-${message.id}`,
            role: "user",
            conversationid: container.conversationid,
            content: [createMessage(generation.prompt)],
            activeMessage: 0,
            availableNextContainer: [],
            activeNextContainer: null,
            previousContainer: container.previousContainer,
          });
        }
        ...
        if (message.type !== "error") {
          ...
          void import("@/features/Misc/reply-completion-notifier").then(({ notifyReplyCompleted }) =>
            notifyReplyCompleted({
              title: "Pulsar",
              body: message.content.slice(0, 120) || "回复已完成。",
            }),
          );
        }
      } catch (error) {
        console.error("[PulsarAI generation] failed", error);
        message.type = "error";
