...
export function mediaIdFromLink(value: string) {
	return value.startsWith(prefix) ? value.slice(prefix.length) : "";
}

export function mediaLinks(value: string) {
	return [...value.matchAll(/media:\/\/([0-9a-f-]{36})/gi)].map(
		(match) => mediaLink(match[1]!),
	);
}

export function isTextMediaType(mediaType: string) {
	return /^text\//i.test(mediaType) || /^application\/(json|xml|javascript)$/i.test(mediaType);
}
...
export async function writeMedia(
	...
	return host.media.write({ bytes: [...bytes], mediaType, ...(path ? { path } : {}) });
}
...
export async function storeDataUrl(value: string, path?: string) {
