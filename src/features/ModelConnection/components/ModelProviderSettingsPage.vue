                ...
                >
                  <div class="flex min-w-0 items-center gap-3">
                    ...
                    <div class="min-w-0">
                      ...
                      <div v-if="model.knowledgeCutoff || modelCapabilities(model).length || priceSummary(model.pricing)" class="mt-1.5 flex flex-wrap gap-x-3 gap-y-1 text-xs text-muted-foreground">
                        <span v-if="model.knowledgeCutoff">知识截至 {{ model.knowledgeCutoff }}</span>
                        <span v-if="priceSummary(model.pricing)">价格：{{ priceSummary(model.pricing) }}</span>
                        <span v-if="modelCapabilities(model).length">能力：{{ modelCapabilities(model).join("、") }}</span>
                      </div>
                    </div>
                  </div>
