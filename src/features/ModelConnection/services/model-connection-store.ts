...
import {
	...
	unregisterProviderHydration,
} from "./model-ai";
import {
	loadPersistedProviders,
	persistProvider,
} from "./model-provider-persistence";
import {
	clearSecretValue,
	deleteSecret,
	hasSecret,
...
function mergeProvider(
	base: ModelProviderDefinition | undefined,
	persisted: ModelProviderDefinition,
): ModelProviderDefinition {
	...
	return {
		...base,
		...persisted,
		...(base?.builtIn ? { builtIn: true, transport: base.transport } : {}),
...
export const useModelConnectionStore = defineStore("modelConnection", {
	...
	getters: {
		...
		visibleModelTabs(): Array<ModelApiType | "all"> {
			const types = new Set(
				this.activeProvider.models.map((model) => model.apiType),
			);
			return [
				"all",
				...Object.keys(modelTypeLabels).filter((type) =>
					types.has(type as ModelApiType),
				),
			] as Array<ModelApiType | "all">;
		},
		chatModelOptions(): Array<{
			...
			return this.providers.flatMap((provider) =>
				provider.models
					.filter(
						(model) =>
							provider.enabled && model.enabled && model.apiType === "chat",
					)
					.map((model) => ({
	...
	actions: {
		async initialize() {
			...
			initializationPromise ??= (async () => {
				...
				for (const provider of persistedProviders) {
					merged.set(
						provider.id,
						mergeProvider(merged.get(provider.id), provider),
					);
				}
		...
		async patchProvider(
			providerId: string,
			patch: Partial<ModelProviderDefinition>,
		) {
