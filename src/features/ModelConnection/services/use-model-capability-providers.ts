...
export function useModelCapabilityProviders(apiType: ModelApiType) {
	...
	const modelProviders = computed(() =>
		store.providers.filter(
			(provider) =>
				...
				provider.models.some((model) => model.apiType === apiType),
		),
	);
		...
		modelProviders.value.map((provider) => ({
			...
			enabled:
				...
				provider.models.some(
					(model) => model.apiType === apiType && model.enabled,
				),
			source: "model",
		})),
	);
	...
	function syncModel() {
		if (
			!models.value.some(
				(model) => model.id === selectedModelId.value && model.enabled,
			)
		) {
			selectedModelId.value =
				models.value.find((model) => model.enabled)?.id ??
				models.value[0]?.id ??
				"";
		}
	}
	...
	async function activateProvider(providerId: string) {
		if (!modelProviders.value.some((provider) => provider.id === providerId))
			return;
	...
	const persistApiKey = useDebounceFn(
