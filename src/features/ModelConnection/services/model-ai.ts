...
type ProviderFactory = { [name: string]: unknown } & ((modelId: string) => unknown);
...
function createNativeProviderBuilder(
  ...
  return (config) => {
    const provider = (createProvider as (settings: Record<string, unknown>) => ProviderFactory)({
      apiKey: `<<${config.apiKeyName}>>`,
      ...(config.baseURL ? { baseURL: config.baseURL } : {}),
      fetch: modelProxyFetch,
      ...extraSettings?.(config),
    });
    const create = (...names: string[]) => (modelId: string) => {
...
const providerConfigs: Record<string, ProviderHydrationConfig> = {
  openai: {
    ...
    kindMap: { chat: "chat", image: "image", embedding: "embedding", asr: "transcription", tts: "speech" },
  },
  deepseek: {
...
export function registerProviderHydration(provider: Pick<ModelProviderDefinition, "id" | "baseUrl" | "apiKeyName" | "transport">) {
  const builder = provider.transport === "ai-sdk" ? nativeProviderBuilders[provider.id] : undefined;
  if (builder) {
...
export function unregisterProviderHydration(providerId: string) {
  if (nativeProviderBuilders[providerId] || providerId === "huggingface") return;
...
export function hydrateModel(model: HydratableModel, kind: ModelKind = "chat") {
  ...
  if (!factory) throw new Error(`Provider ${providerId} does not support ${kind} models.`);
...
function hydrateChatOptions<T extends { model: HydratableModel }>(options: T) {
  ...
  return {
    ...options,
    ...(parsed?.reasoning && input.reasoning === undefined ? { reasoning: parsed.reasoning } : {}),
...
export function embed(options: WithModel<Parameters<typeof baseEmbed>[0]>) {
  return baseEmbed({ ...options, model: hydrateModel(options.model, "embedding") as EmbeddingModel });
}

export function embedMany(options: WithModel<Parameters<typeof baseEmbedMany>[0]>) {
  return baseEmbedMany({ ...options, model: hydrateModel(options.model, "embedding") as EmbeddingModel });
}

export function generateImage(options: WithModel<Parameters<typeof baseGenerateImage>[0]>) {
  return baseGenerateImage({ ...options, model: hydrateModel(options.model, "image") as ImageModel });
}

export function generateSpeech(options: WithModel<Parameters<typeof baseGenerateSpeech>[0]>) {
  return baseGenerateSpeech({ ...options, model: hydrateModel(options.model, "tts") as SpeechModel });
}

export function transcribe(options: WithModel<Parameters<typeof baseTranscribe>[0]>) {
  return baseTranscribe({ ...options, model: hydrateModel(options.model, "asr") as TranscriptionModel });
}
