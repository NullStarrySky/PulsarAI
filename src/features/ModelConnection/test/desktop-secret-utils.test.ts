...
import {
	hydrateSecretPlaceholders,
	secretPreview,
} from "../../../../host/desktop-electron/secret-utils.mjs";
...
describe("Electron secret handling", () => {
	...
	it("hydrates every placeholder from the current secret value", async () => {
		...
		await expect(
			hydrateSecretPlaceholders("Bearer <<provider_API_KEY>>", readSecret),
	...
	it("reports a missing secret instead of forwarding the placeholder", async () => {
		await expect(
			hydrateSecretPlaceholders("<<missing>>", async () => null),
