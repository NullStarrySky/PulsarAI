...
export function parseModelReference(reference: string): ParsedModelReference {
  ...
  const possibleReasoning = segments[segments.length - 1] as ReasoningEffort | undefined;
  const reasoning = possibleReasoning && reasoningEfforts.has(possibleReasoning)
    ? possibleReasoning
    : undefined;
...
export function thinkingLevelLabel(level: ThinkingLevel) {
  return thinkingLevelOptions.find((option) => option.value === level)?.label ?? "自动";
}
