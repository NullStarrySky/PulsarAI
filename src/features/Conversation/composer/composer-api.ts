...
export function createComposerApi(conversationId: string) {
  ...
  const chat = () => chatStore.chats.find((item) => item.id === conversationId) ?? null;
  const send = async () => {
    const target = chat();
    const content = target?.composerDraft?.trim() ?? "";
    if (!target || !content) return null;
    const container = await messageStore.append({ conversationId, role: "user", content, previousContainer: target.lastContainerId });
  ...
  return Object.freeze({
    ...
    write: (content: string) => chatStore.setComposerDraft(String(content), conversationId),
