...
import type { CharacterPackage, ChatMessageContainer, Conversation, PackageCategory } from "@/features/Conversation/messages/conversation-types";
...
export function useConversationStore(pinia?: Pinia): ConversationIntegration {
  ...
  const api = {
    ...
    openPackage: (packageId: string) => packages.packages.find((item) => item.id === packageId) ?? null,
    openConversation: (conversationId: string) => chats.chats.find((item) => item.id === conversationId) ?? null,
  ...
  return Object.defineProperties(api, {
    packages: { get: () => packages.packages, set: (value) => { packages.packages = value; } },
    categories: { get: () => packages.categories, set: (value) => { packages.categories = value; } },
    conversations: { get: () => chats.chats, set: (value) => { chats.chats = value; } },
    containers: { get: () => messages.containers, set: (value) => { messages.containers = value; } },
    loaded: { get: () => true },
