...
const resourceUpdate = computed(() => props.message?.meta.resourceUpdate);
const resourceSummary = computed(() => {
  const update = resourceUpdate.value;
  if (!update) return "";
  const stats = update.stats;
  if (!stats) return `${update.operations.length} 项更新`;
  return `${stats.total} 项 · ${stats.codeAct.committed}/${stats.codeAct.attempted} 已提交`;
});
  ...
  <Message :align="container.role === 'user' ? 'end' : 'start'" class="group/message flex-col gap-1">
    <MessageContent :class="container.role === 'user' ? 'max-w-[77%] self-end mobile:max-w-[88%]' : 'w-full'">
      ...
      <MessageFooter v-if="message" data-window-drag-block class="mt-1 flex h-7 w-full justify-between gap-2 px-0 transition-opacity" :class="container.role === 'assistant' ? 'opacity-100' : 'opacity-0 group-hover/message:opacity-100 group-focus-within/message:opacity-100 mobile:opacity-100'">
        ...
        <div class="flex shrink-0 items-center gap-2 text-xs text-muted-foreground"><Popover v-if="resourceUpdate && (resourceUpdate.operations.length || resourceUpdate.stats?.codeAct.attempted)"><PopoverTrigger as-child><Button variant="ghost" size="icon-sm" class="h-6 gap-1 rounded-full px-1.5 text-[11px] text-amber-500 hover:bg-amber-500/10 hover:text-amber-600" title="查看消息的资源操作统计"><Database class="size-3" /><span>{{ resourceSummary }}</span></Button></PopoverTrigger><PopoverContent align="end" class="w-[min(28rem,calc(100vw-1rem))] space-y-3 p-3 text-xs"><div class="flex items-center gap-1.5 border-b pb-1.5 font-semibold text-amber-500"><Database class="size-4" />消息资源 Overlay</div><div v-if="resourceUpdate.stats" class="grid grid-cols-2 gap-x-4 gap-y-1 rounded-lg bg-muted/50 p-2 font-mono text-[11px]"><span>操作 {{ resourceUpdate.stats.total }}</span><span>日志 {{ resourceUpdate.stats.logCount }}</span><span>提交 {{ resourceUpdate.stats.codeAct.committed }}/{{ resourceUpdate.stats.codeAct.attempted }}</span><span>回滚 {{ resourceUpdate.stats.codeAct.rolledBack }}</span><span>编辑 {{ resourceUpdate.stats.edit }}</span><span>创建 {{ resourceUpdate.stats.create }}</span><span>移动 {{ resourceUpdate.stats.move }}</span><span>删除 {{ resourceUpdate.stats.remove }}</span></div><div v-if="resourceUpdate.operations.length" class="max-h-64 space-y-2 overflow-y-auto font-mono"><pre v-for="(operation, index) in resourceUpdate.operations" :key="index" class="whitespace-pre-wrap break-words rounded border bg-muted/60 p-2">{{ formatValue(operation) }}</pre></div><p v-else class="text-muted-foreground">所有资源操作已随失败的 CodeAct 回滚。</p></PopoverContent></Popover><span class="tabular-nums">{{ messageTime() }}</span></div>
      </MessageFooter>
    </MessageContent>
  </Message>
</template>
