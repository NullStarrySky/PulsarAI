...
const messageContainerTable = "resource_message_containers";
...
function createMessage(
	...
	return {
		...
		content:
			type === "error"
				? formatChatMessageError(content)
...
export const useMessageStore = defineStore("conversation-messages", {
	...
	actions: {
		...
		async loadForChat(chatId: string) {
			...
			const rows = await selectByField<ChatMessageContainer>(
				messageContainerTable,
				"conversationid",
				chatId,
			);
			this.containers = [
				...this.containers.filter((item) => item.conversationid !== chatId),
				...rows.map((row) => normalizeContainer(row.value)),
			];
		...
		async persist(container: ChatMessageContainer) {
			await upsert(
				messageContainerTable,
				container.id,
				structuredClone(toRaw(container)),
			);
		},
		pathFor(containerId: string | null): ChatMessageContainer[] {
			...
			let current = containerId
				? this.containers.find((item) => item.id === containerId)
				: undefined;
			while (current && !seen.has(current.id)) {
				...
				current = current.previousContainer
					? this.containers.find(
							(item) => item.id === current?.previousContainer,
						)
					: undefined;
			}
			return path;
		...
		async appendAssistantVersion(containerId: string) {
			const container = this.containers.find((item) => item.id === containerId);
			if (container?.role !== "assistant") return null;
			container.content.push(createMessage());
			container.activeMessage = container.content.length - 1;
			await this.persist(container);
			return {
				container,
				message: container.content[container.activeMessage]!,
			};
		},
		...
		async deleteContainer(containerId: string) {
			...
			if (previous) {
				...
				if (previous.activeNextContainer === containerId)
					previous.activeNextContainer = nextContainers[0]?.id ?? null;
