...
export function useMessageBubble(
	...
	const conversation = useConversation(
		computed(() => unref(container).conversationid),
	);
	const editing = reactive({ active: false, content: "" });
	const thinking = computed(
		() =>
			unref(message)?.meta.steps.filter(
				(step) =>
					step.type === "thinking" ||
					step.type === "tool-call" ||
	...
	const attachments = computed(
		() =>
			unref(message)?.parts?.filter(
				(part): part is FilePart => part.type === "file",
			) ?? [],
	);
	const worldUpdates = computed(() => unref(message)?.meta.worldUpdates ?? []);
	const hasPluginChanges = computed(() => Boolean(worldUpdates.value.length));
	const agentWorking = computed(
		() =>
			conversation.generating.value &&
			conversation.activePath.value[conversation.activePath.value.length - 1]
				?.id === unref(container).id &&
			unref(message)?.meta.generateInfo?.timeUsed === undefined,
	);
	const hasVisibleMessage = computed(() =>
		Boolean(
			thinking.value.length ||
				attachments.value.length ||
				unref(message)?.content.trim() ||
				unref(message)?.type === "error",
		),
	);
	const canNavigateNext = computed(
		() =>
			(unref(container).activeMessage ?? 0) <
				unref(container).content.length - 1 || !conversation.generating.value,
	);
	const resourceSummary = computed(
		() => `${worldUpdates.value.length} 项资源变更`,
	);
