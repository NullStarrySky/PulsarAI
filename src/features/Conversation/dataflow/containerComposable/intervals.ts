...
import type { IntervalDefinition, IntervalOperation } from "../interval-services";
...
export function useContainerIntervals(source: MaybeRef<ChatContainer | null | undefined>) {
	const container = computed(() => unref(source));
	const message = computed(() => currentMessage(container.value));
	const operations = computed(() => message.value?.meta.intervalOperations ?? []);
	...
	function open(interval: IntervalDefinition) {
		if (!interval.id.trim() || !interval.type.trim()) throw new Error("Interval 必须包含非空 id 和 type。");
		if (interval.autoEndAfter !== undefined && (!Number.isInteger(interval.autoEndAfter) || interval.autoEndAfter < 1)) throw new Error("Interval autoEndAfter 必须是正整数。");
		if (interval.content !== undefined && !isJsonValue(interval.content)) throw new Error("Interval content 必须是 JsonValue。");
		return append({ kind: "interval.open", interval: structuredClone(interval) });
	}
