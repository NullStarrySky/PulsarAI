...
const chatTable = "resource_conversations";
...
export const useChatStore = defineStore("conversation-chats", {
	...
	getters: {
		isGenerating: (state) => (chatId: string) =>
			state.generatingChatIds.includes(chatId),
		chatsForPackage: (state) => (packageId: string) =>
			state.chats
	...
	actions: {
		...
		async loadForPackage(packageId: string) {
			...
			const rows = await selectByField<Conversation>(
				chatTable,
				"packageId",
				packageId,
			);
			this.chats = [
				...this.chats.filter((item) => item.packageId !== packageId),
				...rows.map((row) => normalizeChat(row.value)),
			];
		...
		async load(chatId: string) {
			...
			if (!this.loadedPackageIds.includes(normalized.packageId))
				this.loadedPackageIds.push(normalized.packageId);
			return normalized;
		},
		async persist(chat: Conversation) {
			await upsert(chatTable, chat.id, structuredClone(toRaw(chat)));
		},
		async create(input: {
			packageId: string;
			title?: string;
			...
			const chat: Conversation = {
				...
				binding: input.binding
					? structuredClone(toRaw(input.binding))
					: undefined,
			...
			await Promise.all([
				this.persist(chat),
				useMessageStore().persist(root),
				...(packageItem ? [packages.persist(packageItem)] : []),
			]);
			return chat;
		},
		async update(
		...
		async remove(chatId: string) {
			...
			const chat = this.chats.find((item) => item.id === chatId);
			if (!chat) return;
			const ids = messages.containers
				.filter((item) => item.conversationid === chatId)
				.map((item) => item.id);
		...
		startGeneration(chatId: string) {
			if (chatId && !this.generatingChatIds.includes(chatId))
				this.generatingChatIds.push(chatId);
		},
		finishGeneration(chatId: string) {
			this.generatingChatIds = this.generatingChatIds.filter(
				(id) => id !== chatId,
			);
		},
	},
...
function normalizeChat(item: Conversation): Conversation {
	return {
		...
		composerDraft: item.composerDraft ?? "",
	};
}
