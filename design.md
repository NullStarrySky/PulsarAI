...
- Referenced containers remain lazy namespaces with `get`, `use`, and `list`; `delivery.mode: on_demand` renders only the explicitly referenced descriptor/index and routes full reads through `ctx.containers.retrieve`. Extractors select current-member IDs, transformers receive only authorized contents, and every result retains IDs and paths. Skills use the runtime virtual `container:system/skills` adapter.
- `container:depth/N` is a virtual membership target rather than a `containers.json` declaration. Generation anchors every non-negative depth against the same pre-insertion message list, inserts depth 0 at the bottom, orders members by normal resource priority, and applies Regex only after these role-preserving blocks are present.
